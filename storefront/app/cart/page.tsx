  ...
  return <main className="store-page">
    ...
    <section className="cart-shell">
      ...
      {!lines.length ? <div className="empty-cart"><h2>Your bag is empty.</h2><p>Explore the opening Zucero collection.</p><Link className="button button-dark" href="/#products">Shop products</Link></div> : <div className="cart-layout">
        ...
        <aside className="order-summary"><p className="eyebrow">Order summary</p><div><span>Subtotal</span><strong>{formatPrice(subtotalPaise)}</strong></div><div><span>GST &amp; shipping</span><span>Confirmed by our team</span></div><p className="summary-note">Continue to WhatsApp to confirm availability, delivery charges and payment. No order is placed automatically.</p><a className="button button-dark checkout-button" href={whatsappOrder(lines)}>Order on WhatsApp</a><Link className="text-link continue-link" href="/#products">Continue shopping</Link></aside>
      </div>}
    </section><SiteFooter />
  </main>;
}
