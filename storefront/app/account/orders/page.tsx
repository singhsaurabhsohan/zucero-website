  ...
  return <main className="store-page"><StoreHeader /><section className={styles.shell}>
    ...
    <div className={styles.grid}>
      ...
      <div className={styles.content}>
        ...
        {section === "orders" && <div className={styles.panel}><div className={styles.panelHead}><div><h2>Your orders</h2><p>Live delivery status and tracking for purchases placed with {email}.</p></div></div>{!orders.length ? <div className={styles.empty}><p>No orders to show yet.</p><Link className="button button-dark" href="/products">Explore products</Link></div> : <div className={styles.orderList}>{orders.map(order => { const progress = orderProgress(order.display_status); return <article className={styles.orderCard} key={order.id}><div className={styles.orderHead}><div><p className="eyebrow">{new Date(order.created_at).toLocaleDateString("en-IN", {day:"numeric",month:"short",year:"numeric"})}</p><h3>{order.order_number}</h3></div><span className={styles.status}>{order.display_status}</span></div><div className={styles.items}>{order.items.map(item => <div className={styles.itemRow} key={`${order.id}-${item.sku}`}><span>{item.product_name} · {item.variant_label} × {item.quantity}</span><strong>{formatPrice(item.line_total_paise)}</strong></div>)}</div><div className={styles.money}><div><span>Shipping</span><strong>{order.shipping_paise ? formatPrice(order.shipping_paise) : "Free"}</strong></div><div><span>GST</span><strong>{formatPrice(order.tax_paise)}</strong></div><div><span>Total paid</span><strong>{formatPrice(order.total_paise)}</strong></div></div><div className={styles.timeline} aria-label={`Order progress: ${order.display_status}`}>{[1,2,3,4,5].map(step => <span key={step} className={`${styles.step} ${progress >= step ? styles.done : ""}`}/>)}</div><div className={styles.tracking}><p><strong>Payment:</strong> {order.payment_status === "captured" ? "Paid" : order.payment_status}</p>{order.tracking_awb ? <><p><strong>Courier:</strong> {order.courier_name || "Assigned"}</p><p><strong>AWB:</strong> {order.tracking_awb}</p>{order.tracking_url && <a className="button button-dark" href={order.tracking_url} target="_blank" rel="noreferrer">Track shipment</a>}</> : <p><strong>Delivery:</strong> Your order is being prepared. Tracking will appear after courier assignment.</p>}</div></article>; })}</div>}</div>}
      </div>
    </div>
