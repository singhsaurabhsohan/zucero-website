...
export default function CheckoutPage() {
  ...
  return <main className="store-page checkout-page"><StoreHeader /><section className="checkout-layout">
    <form className="checkout-form" onSubmit={submit}>
      <div className="checkout-heading"><p className="eyebrow">Secure checkout</p><h1>Where should we send it?</h1><p>Enter your India delivery address. GST and shipping update automatically before Razorpay opens. Delivery is usually within ~7 days.</p></div>
      ...
      <p className="note">Shipping is ₹79 within Haryana, ₹129 to the rest of India, and free on product subtotals of ₹1,499 or more. We verify courier serviceability with Shiprocket before payment. Delivery is usually within ~7 days.</p>
    ...
    <aside className="checkout-summary"><p className="eyebrow">Your order</p>{lines.map((line) => <div className="checkout-line" key={line.variantId}><span>{line.productName} · {line.variantLabel} × {line.quantity}</span><strong>{formatPrice(line.pricePaise * line.quantity)}</strong></div>)}<div className="checkout-line"><span>Product subtotal</span><strong>{formatPrice(subtotalPaise)}</strong></div>{quote ? <><div className="checkout-line"><span>Shipping</span><strong>{quote.shippingPaise === 0 ? "Free" : formatPrice(quote.shippingPaise)}</strong></div>{quote.mode === "CGST_SGST" ? <><div className="checkout-line"><span>CGST @ 2.5%</span><strong>{formatPrice(quote.cgstPaise)}</strong></div><div className="checkout-line"><span>SGST @ 2.5%</span><strong>{formatPrice(quote.sgstPaise)}</strong></div></> : <div className="checkout-line"><span>IGST @ 5%</span><strong>{formatPrice(quote.igstPaise)}</strong></div>}<div className="checkout-total"><span>Total payable</span><strong>{formatPrice(quote.totalPaise)}</strong></div><p className="note">Delivery is usually within ~7 days after order confirmation.</p></> : <p>Select your delivery state to calculate GST and shipping.</p>}</aside>
